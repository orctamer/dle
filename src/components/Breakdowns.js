    ...
    return (
        <main className="bg-blue-100 min-h-screen p-12">
            <section className="container mx-auto">
                <h1 className="text-5xl flex justify-center mb-12">Breakdowns</h1>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {postData && postData.map((post, index) => (                                               
                    <article className="bg-white rounded-lg" key={index}>
                        <Link to={"/breakdown/" + post.slug.current } key={post.slug.current}>                        
                        <span className="block h-64 relative rounded shadow leading-snug bg-white border-l-l8 border-blue-400" key={index}>
                        <img src={urlFor(post.image).url()} alt={post.image.alt} className="h-full w-full rounded-t-lg object-cover absolute" />                            
                        </span>
                        </Link>
                        <h1 className="text-4xl py-2 text-center">{post.name}</h1>
